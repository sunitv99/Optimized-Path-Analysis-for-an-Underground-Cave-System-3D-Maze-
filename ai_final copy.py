  ...
  while queue:
    ...
    if current in d and visited[current]==0:
      ...
      for i in range(len(d[current])):
        ...
        # print(t)
        # print(queued)
        if t not in queued and visited[t]==0 and withinLimit(t):
          # print(current,t)
          queued[t]=current
          queue.append(t)   
        if t==end:
...
def Astar(start):
  ...
  while queue:
    ...
    costl= c[4]
    # print(current)
    if current in ascoredict and ascoredict[current]<ascore:
      continue
    if current == end and costf>=cost1:
      f=1
      costf=cost
    ...
    if current in d and visited[current]==0:
      ...
      for i in range(len(d[current])):
        ...
        print(t,a1,queued)
        print(ascoredict)
        if t in queued and ascoredict[t]>a1:
          cl2=costl.copy()
          if d[current][i]>=0 and d[current][i]<=6:
            cl2.append(10)
          else:
          ...
          ascoredict[t]=a1
          queued[t]=current
          heappush(queue,[a1, cnt, t, c1, cl2])
        if t not in queued and visited[t]==0 and withinLimit(t):
          queued[t]=current
          cl=costl.copy()
          if d[current][i]>=0 and d[current][i]<=6:
